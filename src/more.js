...
/**
 * Turns a filename into an id-safe token: numbering stripped, anything that is not a
 * letter, digit or underscore replaced with an underscore, lower-cased.
 *
 * @method     cleanID
 * @private
 * @param      {string}   id        - The filename or folder name.
 * @param      {boolean}  amFolder  - True for a folder, which has no extension to strip.
 * @return     {string}             - The id segment, e.g. "quick_reference".
 */
...
/**
 * Builds the "more" pages and merges them into the menu. See the class description
...
/**
 * Derives the id a "more" file will be filed under, without building anything.
 *
 ...
 */
module.exports.pageId = function(relative, delimiter){

	var previous = quirkDelimiter;
	if(delimiter){
		quirkDelimiter = delimiter;
	}

	var parts    = ("more/" + relative).split("/");
	var filename = parts.pop();
	var out      = [];

	for(var i=0; i<parts.length; i++){
		out.push( cleanID(parts[i], true) );
	}
	out.push( cleanID(filename, false) );

	quirkDelimiter = previous;
