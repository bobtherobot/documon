...
Documon accepts the common JSDoc spellings as exact synonyms — `@function`/`@func` for
`@method`, `@arg`/`@argument` for `@param`, `@prop`/`@member` for `@property`,
`@augments` for `@extends`, `@constructs` for
`@constructor`, and `@const`/`@constant` for a read-only `@property`. `@access private`
becomes `@private`.

Description tags (`@desc`, `@description`, `@summary`, `@classdesc`, `@fileoverview`) are
folded into the block description, and inline `{@link target}` becomes a markdown link.

`@deprecated`, `@throws`, `@since`, `@author`, `@license`, `@copyright` and `@todo` have no
structural role, but are rendered as metadata rows on the member rather than discarded.

Run `documon --check` to see exactly what was normalized or ignored.

## Retired spellings

`@impliments` was Documon's own misspelling of `@implements`. It was retired in v2.7.0
